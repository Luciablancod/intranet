import { Request, Response } from "express";
import { EventDateService } from "../services/event_date.service";

const service = new EventDateService();

export class EventDateController {
    static async get(req: Request, res: Response): Promise<Response> {
        try {
            const response = await service.findAll();
            return res.json({ success: true, data: response });
        } catch (error) {
            if (error instanceof Error) {
                return res.status(500).json({ message: "Internal Server Error", error: error.message });
            }
            return res.status(500).json({ message: "Internal Server Error" });
        }
    }

    static async getById(req: Request, res: Response): Promise<Response> {
        try {
            const { id } = req.params;
            const eventDate = await service.findById(Number(id));

            if (!eventDate) {
                return res.status(404).json({ message: "Event date not found" });
            }

            return res.json({ success: true, data: eventDate });
        } catch (error) {
            if (error instanceof Error) {
                return res.status(500).json({ message: "Internal Server Error", error: error.message });
            }
            return res.status(500).json({ message: "Internal Server Error" });
        }
    }

    static async create(req: Request, res: Response): Promise<Response> {
        try {
            const newEventDate = await service.create(req.body);
            return res.status(201).json({ success: true, data: newEventDate });
        } catch (error) {
            if (error instanceof Error) {
                return res.status(400).json({ message: "Bad request", error: error.message });
            }
            return res.status(400).json({ message: "Bad request" });
        }
    }

    static async update(req: Request, res: Response): Promise<Response> {
        try {
            const { id } = req.params;
            const updatedDate = await service.update(Number(id), req.body);
            return res.json(updatedDate);
        } catch (error) {
            if (error instanceof Error) {
                return res.status(400).json({ message: "Bad request", error: error.message });
            }
            return res.status(400).json({ message: "Bad request" });
        }
    }

    static async _delete(req: Request, res: Response): Promise<Response> {
        try {
            const { id } = req.params;
            await service.delete(Number(id));
            return res.json({ message: "Event date deleted successfully" });
        } catch (error) {
            if (error instanceof Error) {
                return res.status(500).json({ message: "Internal Server Error", error: error.message });
            }
            return res.status(500).json({ message: "Internal Server Error" });
        }
    }
}